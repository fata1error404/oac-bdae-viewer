		...
		for (int j = 0; j < tilesZ; j++)
		{
			...
			for (int col = 0; col < UnitsInTileCol; col++)
			{
				for (int row = 0; row < UnitsInTileRow; row++)
				{
					...
					ChunkInfo &chunk = tile->chunks[row + col / 8];
					float texIdx1 = chunk.texNameIndex1;
					float texIdx2 = chunk.texNameIndex2;
					float texIdx3 = chunk.texNameIndex3;
			...
			{
				int globalTextureIndex = tile->textureIndices[i];
