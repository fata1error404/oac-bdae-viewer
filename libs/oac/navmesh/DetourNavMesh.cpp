...
#define NAV_MESH_MAX_TILE_INDEX 128
...
dtNavMesh::~dtNavMesh()
{
	...
	while (m_nodePools.size() > 0)
	{
		delete m_nodePools.back();
		m_nodePools.pop_back();
	}
	while (m_openLists.size() > 0)
	{
		delete m_openLists.back();
		m_openLists.pop_back();
	}
	pthread_mutex_unlock(&m_cacheMutex);

	pthread_mutex_destroy(&m_cacheMutex);
}
...
dtTileRef dtNavMesh::addTile(unsigned char *data, int dataSize, int flags, dtTileRef lastRef)
{
	...
	if (!lastRef)
	{
		int h = (header->y + NAV_MESH_MAX_TILE_INDEX) * NAV_MESH_MAX_TILE_INDEX * 2 + (header->x + NAV_MESH_MAX_TILE_INDEX);
...
dtMeshTile *dtNavMesh::getTileAt(int x, int y) const
{
	...
	if ((x >= -NAV_MESH_MAX_TILE_INDEX && x < NAV_MESH_MAX_TILE_INDEX) && (y >= -NAV_MESH_MAX_TILE_INDEX && y < NAV_MESH_MAX_TILE_INDEX))
	{
		int h = (y + NAV_MESH_MAX_TILE_INDEX) * NAV_MESH_MAX_TILE_INDEX * 2 + (x + NAV_MESH_MAX_TILE_INDEX);
		dtMeshTile *tile = m_tiles + h;
						...
						dtPolyRef *path, const int maxPathSize) const
{
	...
	// Change Node Pool and OpenList to stack variables for multi-thread

	dtNodePool *nodePool = NULL;
	dtNodeQueue *openList = NULL;
	newNodePoolAndOpenList(nodePool, openList);
	if (nodePool == NULL || openList == NULL)
		return 0;
	nodePool->clear();
	openList->clear();
	...
	deleteNodePoolAndOpenList(nodePool, openList);
								...
								unsigned char &fromType, unsigned char &toType) const
{
	...
	if (ip >= (unsigned int)fromTile->header->polyCount)
		return false;
	...
	if (ip >= (unsigned int)toTile->header->polyCount)
		return false;
...
void dtNavMesh::newNodePoolAndOpenList(dtNodePool *&nodePool, dtNodeQueue *&openList) const
{
	pthread_mutex_lock(&m_cacheMutex);
	nodePool = NULL;
	openList = NULL;
	if (m_nodePools.size() > 0)
	{
		nodePool = m_nodePools.back();
		m_nodePools.pop_back();
	}
	if (m_openLists.size() > 0)
	{
		openList = m_openLists.back();
		m_openLists.pop_back();
	}
	if (nodePool == NULL)
	{
		nodePool = new dtNodePool(MAX_NODES, dtNextPow2(MAX_NODES / 4));
	}
	if (openList == NULL)
	{
		openList = new dtNodeQueue(MAX_NODES);
	}
	pthread_mutex_unlock(&m_cacheMutex);
}

void dtNavMesh::deleteNodePoolAndOpenList(dtNodePool *&nodePool, dtNodeQueue *&openList) const
{
	pthread_mutex_lock(&m_cacheMutex);
	if (m_nodePools.size() < MAX_CACHE_SIZE)
		m_nodePools.push_back(nodePool);
	else
		delete nodePool;
	nodePool = NULL;
	if (m_openLists.size() < MAX_CACHE_SIZE)
		m_openLists.push_back(openList);
	else
		delete openList;
	openList = NULL;
	pthread_mutex_unlock(&m_cacheMutex);
