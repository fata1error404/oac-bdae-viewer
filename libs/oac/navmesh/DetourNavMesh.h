...
private:
	...
	void newNodePoolAndOpenList(dtNodePool*& nodePool, dtNodeQueue*& openList) const;
	void deleteNodePoolAndOpenList(dtNodePool*& nodePool, dtNodeQueue*& openList) const;
	mutable std::deque<dtNodePool*> m_nodePools;
	mutable std::deque<dtNodeQueue*> m_openLists;
	mutable pthread_mutex_t m_cacheMutex;
};
